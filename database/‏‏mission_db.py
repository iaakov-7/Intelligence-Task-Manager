...
class MissionDB:
    ...
    def update_mission_status(self,id:int, status:str):
        ...
        if status == "IN_PROGRES" and status_mission != "ASSIGNED":
            raise MissionStatusError
        if status == "COMPLETED" or status == "FAILED" and status_mission != "IN_PROGRES":
            raise MissionStatusError
        ### לעדכן אצל הסוכן במשימות שהצליחו או נכשלו ולעשות רייס לביטול משימה
    ...
    def get_open_missions_by_agent(self,id:int):
        conn = self.db.get_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM missions WHERE assigned_agent_id=%s AND (status='ASSIGNED'OR 'IN_PROGRESS')",(id,)) 
...
print(db_mission.get_open_missions_by_agent(2))
